...
Documented the rule in two places matched to their audiences: one subsection in `docs/agents-context.md` (the canonical source of the shared `AGENTS.md` region, which `arcadia way` propagates to every adopting repository) telling agents to use the commands and why, and the full mechanics in `docs/managed-documents.md`. Regenerated Arcadia's own `AGENTS.md` via `project setup-context --repo .`.
...
- **Next:** Propagate the authoring rule to the three stale adopting repositories (PPN, Rebuster, Martian Rover), which requires either explicit operator approval per repository or an answer to the still-open `propagation-authority` question.
