...
CREATE TABLE IF NOT EXISTS skill_definitions (
  ...
  executor_type TEXT NOT NULL CHECK (executor_type IN ('deterministic', 'codex_planning', 'codex_build', 'operator')),
...
CREATE TABLE IF NOT EXISTS execution_plan_steps (
  ...
  executor_type TEXT NOT NULL CHECK (executor_type IN ('deterministic', 'codex_planning', 'codex_build', 'operator')),
  safe_to_run INTEGER NOT NULL CHECK (safe_to_run IN (0, 1)),
  status TEXT NOT NULL CHECK (status IN ('pending', 'running', 'completed', 'requires_review', 'failed', 'skipped')),
  needs_operator TEXT,
  created_at TEXT NOT NULL,
  updated_at TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS execution_runs (
  ...
  work_item_id TEXT,
  plan_id TEXT,
  status TEXT NOT NULL CHECK (status IN ('pending_execution', 'running', 'completed', 'requires_review', 'failed')),
  summary TEXT NOT NULL,
  mission_log_id TEXT,
  review_item_id TEXT,
  executor_name TEXT,
