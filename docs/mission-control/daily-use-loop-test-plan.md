...
| Expected persisted state | One `ask_requests` record; one linked Action and plan; one initial open Decision; approved Decision before Run starts; one planning Run; completed invocation; Action moved to Requires Review with a final plan-acceptance Decision |
...
| Expected Responsibility | Codex before execution; Requires Review after the validated plan is ready for acceptance |
...
| Expected Responsibility | Requires Review only if promoted or clarified later; no executable Responsibility assigned at capture |
...
| Expected persisted state | Decision remains open; no Run or invocation status transition; approval gates remain pending; Action remains Requires Review |
...
| Expected Responsibility | Requires Review |
...
| Expected persisted state | Run is `requires_review`, not `completed`; Action is Requires Review; one open Validation Decision includes machine-readable failure codes |
...
| Expected Responsibility | Requires Review for final plan acceptance |
...
### Test: Attention mirrors Requires Review and Blocked state after failure

| Field | Specification |
| --- | --- |
| Test name | `Attention mirrors Requires Review and Blocked state after failure` |
...
| Expected Responsibility | Requires Review for Validation failure; Blocked for executor failure |
...
| `dashboard snapshot represents one authoritative recovery item per failed Run` | `buildDashboardSnapshot` | Seed runner failure and worker failure | Read-only; no writes | Attention item links Run, evidence, Log, and Decision where applicable | Requires Review/Blocked labels match SQLite; no duplicate contradiction | Snapshot mapping is best tested directly | `tests/dashboard-snapshot.test.ts` |
...
| `file ingress preserves canonical Decision linkage` | `runIngressProcessCommand` | Real input file with canonical request | Ask, Action, Decision, packet, ingress Log, sidecar all linked | File moves to Done; sidecar exposes Decision and Artifact paths | Requires Review; processing failure moves to Failed with Log | File ingress has extra provenance not covered by browser | `tests/ingress.test.ts` |
| `Discord request preserves canonical Decision identity` | Real `ArcadiaCli` adapter with fake Discord interaction | Seed workspace; submit canonical request | Same Decision ID visible through CLI review | Formatter shows Project, expected Artifact, Decision, boundaries, Next Action | Requires Review; Discord does not run provider directly | Adapter contract check without external Discord | `tests/discord-bot.test.ts` |
...
| `Run outcome reducer never completes on failed Validation` | Worker/runner status reducer | Exit and Validation matrix | None | Deterministic terminal result and summary | Passed→completed; quality failure→Requires Review; execution failure→Blocked | Removes branching ambiguity from worker loop | New worker-status unit test |
...
| `Attention card does not expose provider command` | Snapshot action builder | Packet with open Decision | None | Primary actions are View/Approve/Reject/Defer; provider command absent | Requires Review | Pure view-model contract | `tests/dashboard-snapshot.test.ts` |
...
| `retry creates a new attempt linked to the prior failed Run` | Retry policy/helper | Failed Run with retained evidence | New Run ID with prior-run reference; original immutable | Original Artifacts/Log retained; new attempt visible | Codex or Requires Review according to approved retry | Pure retry contract before UI wiring | New retry policy unit/integration test |
