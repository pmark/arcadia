...
> **Editor's note (2026-07-28):** This branch was never merged or opened as a
> PR; it sat stranded for ~4 weeks before being recovered. In the 97 commits
> since `829cd30`, the review's central recommendation — adopt what exists,
> build nothing new, leave schema and vocabulary alone — was overtaken by
> events: the managed-documentation/dispatch subsystem (`src/docs/*`,
> `docs/plans/`, `CLAUDE.md`) did not exist yet and is now ~2,200 lines of new
> plumbing, plus a dozen new plan documents. **D7 is resolved** — `PROJECT.md`
> now carries a real goal, outcome, and active plan instead of the placeholder
> "Mission needs definition" this review flags in §5.1 and §10. Treat this as
> a dated snapshot of the repository's state and reasoning on 2026-07-02, not
> as current-state fact — several of its "Now" items and D1–D7 decisions may
> already be settled or superseded by later work. Landed as historical record
> because the analysis itself remains sound for its date and cites real
> evidence; nothing below this note has been altered from the original.
...
Help the operator maintain steady, visible progress across many simultaneous creative and software projects with minimal cognitive overhead, by making the next artifact, the next action, and the items that genuinely need their judgment always one glance away — and by executing everything else through the cheapest safe deterministic path.
...
4. **Only judgment reaches the operator.** The Review queue contains only real Decisions (approve/reject/defer/clarify); safe deterministic work runs without them.
...
The canonical vocabulary is defined in `docs/arcadia-semantics.md` (Domain, Project, Mission, Outcome, Milestone, Action, Artifact, Decision, Log; Responsibility values Autonomous / Codex / Needs operator / Blocked; Project statuses Active / Paused / Incubating / Completed) and matches the required project model in this review's brief almost exactly. Legacy schema names (`work_items`, `review_items`, `back_burner_items`, `projects.goal`) are explicitly documented as frozen compatibility names — a deliberate and sound choice.
...
| Needs operator surfacing | **T** | `/review` + Attention built from actual actionable Decisions; CLI `review`/`inbox attention`; Discord Requires Review notifications with dedup | Attention/`/review` parity asserted in e2e test 5 |
...
**V1 acceptance bar (proposed, measurable):** ten consecutive operating days in which (a) each day starts from the command center, (b) at least one capture per day resolves to a completed Run, a Decision, or a deliberate Back Burner park, (c) zero occasions where the operator must open SQLite or read source to answer "what's next," and (d) the Golden Request Suite gained at least five new real-world entries.
...
| Autonomous agents / continuous orchestration | Progress-while-sleeping fantasy | A long record of Decision-gated runs with near-zero false completions; explicit per-project autonomy policy from the operator |
...
- Intent-registry growth: given a week's Back Burner misses, *propose* new registry entries and golden tests as a diff for the operator's review — never live routing.
...
**3. Always Needs operator:** approving any Decision; accepting any plan; publishing, deploying, merging, spending, credentials, messaging (the boundary list already embedded in packets and `config/defaults/operator-context.md`); changing a Project's lifecycle status; deleting or archiving anything.
...
## 10. Executive Decisions for the operator (Review Question F — max 7)
...
*Why now:* the system has never held the operator's real portfolio; every remaining unknown (routing coverage, phone ergonomics, report usefulness) is only discoverable in use. *Default:* the ten-day bar in §7, starting within a week of a green CI baseline. *Easier:* prioritization becomes empirical. *Don't build until decided:* speculative UX work, new adapters.

**D3. Set the capture-fallback policy for unrecognized input.**
*Why now:* deterministic intake will miss a lot of real phrasing at first; today's fallback is Back Burner. *Options:* (a) keep Back Burner + a daily triage pass (recommended — safe, already tested), (b) create a Needs operator Action for every miss (noisier queue), (c) frontier-assisted interpretation behind a Decision (later). *Easier:* defines the triage habit and the registry-growth loop. *Don't build until decided:* any model-in-the-loop capture.
...
| --- | --- | --- | --- | --- | --- | --- |
| N1 | Verify green baseline: run full `pnpm test` + builds + `test:e2e` on `main`; enable branch protection requiring CI | Trustworthy foundation for everything else | Autonomous (run) + Needs operator (protection setting) | CI runs green; protection enabled | Both CI jobs green on `main`; merges blocked on red | — |
| N2 | Seed the real workspace: init with `--profile arcadia`, import full project portfolio with missions, statuses, milestones, next actions, metadata (repo paths, validation commands) | Outcomes 1–3; D2, D7 | Needs operator (content) + Autonomous (commands) | Populated workspace DB; first real `reports/status.md` | `project list` shows every real project with a truthful status and next action | N1 |
| N3 | Ten-day dogfooding trial per §7, with a one-page operating log | Empirical priority signal; v1 acceptance | Needs operator | `docs/reports/v1-dogfooding-log.md` (in workspace or repo) | Bar in §7 met or misses enumerated | N2, D2 |
...
| N6 | Golden-suite ratchet: convert each trial routing miss into `tests/goldenRequests.ts` entries + registry fixes | Growing deterministic coverage (Outcome 2) | Codex (fixes) + Needs operator (which misses matter) | Updated suite + `docs/reports/golden-request-suite.md` addendum | ≥5 new real entries; suite green | N3 |
...
| # | Item | Outcome enabled | Class | Expected artifact | Acceptance criteria | Depends on |
| --- | --- | --- | --- | --- | --- | --- |
| X1 | Stale-project detection: deterministic flag in status/weekly review for Active projects with no Action/Log in N days | Portfolio honesty (Outcome 1) | Codex | Code + tests + report section | Weekly review lists stale Actives with suggested status change (the operator decides) | N3 |
...
| X4 | Weekly frontier synthesis job (drafted narrative over weekly review + Logs) via Intelligence v0.1 | High-leverage model use (§9.1) | Codex (plumbing) + Needs operator (accept each draft) | Job template + drafted Artifact + acceptance Decision | Draft appears as drafted Artifact with Decision; nothing auto-published | N3, D1 |
| X5 | Remote reachability per D4(b) if chosen (Tailscale doc, no code) | True away-from-home phone loop (Outcome 8) | Needs operator (security) + Autonomous (setup) | SETUP.md section | Dashboard reachable from phone off-LAN through private overlay only | D4 |
...
| L4 | Autonomy expansion (unattended model execution) | Blocked | Long false-completion-free record + explicit per-project policy from the operator |
...
- **Recommendations** (sections 7–11 defaults, vocabulary parking, executor consolidation) are labeled as such and are choices for the operator, not repository facts.
