...
convert an occasional silent pointer loss into a frequent one. The fix needs
a compare-and-set on `PROJECT.md`'s content (or a monotonic revision field
checked at write time) so a settlement fails loudly and retries rather than
silently overwriting a pointer another settlement already moved — the same
"fail loudly, never fail silently" discipline `CONSTITUTION.md`'s Truth
section already requires elsewhere in this repository.
