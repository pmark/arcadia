...
actions:
  ...
  - id: establish-minimal-pr-qa
    ...
    acceptance_criteria:
      ...
      - Arcadia selects the least-cost compliant read-only reviewer through the existing coding-agent profile and provider-adapter registries, proves the evidence-readable/home-denied/repository-denied/network-denied boundary with a preserved fail-closed runtime preflight, then invokes it with no arbitrary configured arguments and requires an exhaustively runtime-validated structured verdict with ordered findings and exactly one result for every fixed QA criterion.
