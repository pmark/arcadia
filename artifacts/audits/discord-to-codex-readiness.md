...
| Discord shows Requires Review items | Ready | `apps/discord-bot/src/commands/requiresReview.ts:4-6` reads the compatibility review queue; formatter tests assert "Requires Review" and no legacy label leakage. | Legacy queue aliases remain for persisted data compatibility. | Keep external wording as Requires Review. |
...
| Requires Review terminology | Ready | Dashboard, Discord, CLI, status reports, run output, and docs use Requires Review for user-facing surfaces; tests cover legacy label leakage. | Legacy internal aliases remain for persisted data compatibility. | Keep external wording as Requires Review. |
...
2. User-facing CLI and Markdown output use Requires Review terminology, while legacy persisted values remain supported for compatibility.
