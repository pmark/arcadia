...
function Card({ card }: { card: FlightDeckCard }) { return <article className="border-l-4 border-moss rounded-md border border-line bg-panel p-3 shadow-soft"><p className="text-[10px] font-bold uppercase tracking-wider text-muted">{card.kind}</p><h3 className="mt-1 text-sm font-semibold">{card.title}</h3><p className="mt-1 line-clamp-3 text-xs text-muted">{card.detail}</p>{card.relation !== "direct" ? <p className="mt-2 text-[10px] uppercase tracking-wide text-muted">{card.relation === "unattached" ? "Unattached to a Plan" : "Linked structurally"}</p> : null}</article>; }
